...
// ==UserScript==
// @name         云盘秒传工具（百度/夸克/天翼/123/光鸭）- 修复版
// @version      2026.04.29_Fixed
...
(function () {
    ...
    const SCRIPT_VERSION = "2026.04.29_Fixed";
        ...
        async getFileInfoBatch(idList) {
            ...
            for (let i = 0; i < idList.length; i += batchSize) {
                ...
                try {
                    ...
                    console.warn(`[123云盘] 第 ${i} 批文件获取失败，跳过本批...`, err);
                }
                await helper.sleep(500); // 适度增加延迟防风控
            }
            return rows;
        }
    }
        ...
        async getShareMd5Map(shareId, stoken, cookie, fileItems) {
            ...
            for (let i = 0; i < fileItems.length; i += batchSize) {
                ...
                try {
                    ...
                    console.warn(`[夸克网盘] 第 ${i} 批文件 MD5 获取失败，跳过...`, err);
                    fids.forEach((fid) => (md5Map[fid] = ""));
                }
...
function guangyaExtractMd5FromEtag(raw) {
    ...
    function createButton() {
        ...
        // 【修复2】明确指定挂载点，丢弃容易报错的 querySelector("*")
        /** @type {Element} */
    ...
    function init() {
        ...
        // 【修复1】增加 MutationObserver 防抖，避免导致页面卡死
        let timeoutId = null;
